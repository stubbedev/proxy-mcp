...
// Run is the proxy entry point: it parses flags, loads the config, and either
// validates it or starts the server. version is the build version baked into
// the root main package (so the -X main.BuildVersion ldflag keeps working).
func Run(version string) {
	...
	if err := startHTTPServer(config, *idleTimeout, loadConfig, *conf); err != nil {
